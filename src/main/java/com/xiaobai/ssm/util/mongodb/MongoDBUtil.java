package com.xiaobai.ssm.util.mongodb;
...
public enum MongoDBUtil {
    ...
    static {
        ...
        // 从配置文件中获取属性值
        String ip = config.getString("mongo.host");
        int port = config.getInt("mongo.port");
        //String myUserName = config.getString("mongo.user");
        //String myPassword = config.getString("mongo.pwd");

        instance.mongoClient = new MongoClient(ip, port);

        // 大部分用户使用mongodb都在安全内网下，但如果将mongodb设为安全验证模式，就需要在客户端提供用户名和密码：
        // boolean auth = mogoclint.authenticate(myUserName, myPassword);
